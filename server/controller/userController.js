import express from "express"
import { UserModel } from "../models/User.js"
import { body } from "express-validator"
import { validationResult } from "express-validator"
import bcrypt from "bcrypt"
import dotenv from "dotenv"
dotenv.config({ path: "../config/.env" })



import jwt from "jsonwebtoken"
import { AdminModel } from "../models/AdminModel.js"




const Register = async (req, res) => {


    const errors = validationResult(req)

    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }




    const { name, email, password, gender } = req.body;
    try {
        const userExist = await UserModel.findOne({ email });


        if (userExist) {


            return res.status(400).json({

                errors: [{ msg: "This Email is already Registered" }],

            });






        }

        const hashPassword = await bcrypt.hash(password, 12)
        const newUser = new UserModel({ name, email, password: hashPassword, gender })
        const result = await newUser.save()
        result._doc.password = undefined;

        return res.status(201).json({ success: true, ...result._doc })

    } catch (err) {





        return res.status(500).json({ error: err.message })


    }





};




const Login = async (req, res) => {











    const errors = validationResult(req)

    if (!errors.isEmpty()) {

        return res.status(400).json({ errors: errors.array() });


    }

    const { email, password } = req.body;
    try {
        const userExist = await UserModel.findOne({ email });
        if (!userExist) {



            return res.status(400).json({

                errors: [{ msg: "User Not Registerd" }],


            });



        }



        const isPasswordOk = await bcrypt.compare(password, userExist.password)
        if (!isPasswordOk) {
            return res.status(400).json({
                errors: [{ msg: "Wrong Password" }],
            });

        }


        const token = jwt.sign({ _id: userExist._id }, process.env.JWT_SECRET_KEY, { expiresIn: "3d" })
        const user = { ...userExist._doc, password: undefined }




        return res.status(201).json({ success: true, user, token })

    } catch (err) {

        console.log(err)

        return res.status(500).json({ error: err.message })


    }



};


const Auth = (req, res) => {
    return res.status(200).json({ success: true, user: { ...req.user._doc } })
};


const AuthAdmin = (req, res) => {
    return res.status(200).json({ success: true, user: { ...req.admin._doc } })
};



const GetProfile = async (req, res) => {




    try {
        const profile1 = await UserModel.findOne({ email: req.user.email })



        return res.status(200).json({ success: true, profile1 })

    } catch (err) {


        return res.status(500).json({ error: err.message })
    }

}


const EditProfile = async (req, res) => {







    try {

        const { name, email } = req.body;


        const prev1 = req.user.email
        const prev2 = req.user.name




        if (prev1 === email && prev2 === name) {
            const result = await UserModel.findOneAndUpdate({ _id: req.user._id }, { ...req.body }, { new: true })




            return res.status(200).json({ success: false, ...result._doc })


        }


        const userExist = await UserModel.findOne({ email });



        if (userExist && prev1 !== email) {
            return res.status(400).json({
                errors: [{ msg: "This is a already existinng email" }],
            });
        }










        const result = await UserModel.findOneAndUpdate({ _id: req.user._id }, { ...req.body }, { new: true })




        return res.status(200).json({ success: true, ...result._doc })


    } catch (err) {
        return res.status(500).json({ error: err.message })
    }

}









const AdminRegister = async (req, res) => {


    console.log("innnnnnnnnnnnnnn");

    console.log(req.body);











    const { email, password } = req.body;
    try {
        const userExist = await AdminModel.findOne({ email });


        if (userExist) {


            return res.status(400).json({

                errors: [{ msg: "This Email is already Registered" }],

            });






        }

        const hashPassword = await bcrypt.hash(password, 12)
        console.log(hashPassword);

        const newAdmin = new AdminModel({ email, password: hashPassword })
        console.log(newAdmin);

        const result = await newAdmin.save()
        result._doc.password = undefined;

        return res.status(201).json({ success: true, ...result._doc })

    } catch (err) {





        return res.status(500).json({ error: err.message })


    }





};




const AdminLogin = async (req, res) => {













    const { email, password } = req.body;
    try {
        const userExist = await AdminModel.findOne({ email });
        if (!userExist) {



            return res.status(400).json({

                errors: [{ msg: "User Not Registerd" }],


            });



        }



        const isPasswordOk = await bcrypt.compare(password, userExist.password)
        if (!isPasswordOk) {
            return res.status(400).json({
                errors: [{ msg: "Wrong Password" }],
            });

        }


        const token = jwt.sign({ _id: userExist._id }, process.env.JWT_SECRET_KEY, { expiresIn: "3d" })
        const user = { ...userExist._doc, password: undefined }




        return res.status(201).json({ success: true, user, token })

    } catch (err) {

        console.log(err)

        return res.status(500).json({ error: err.message })


    }



};









export { Register, Login, Auth, GetProfile, EditProfile, AdminRegister , AdminLogin , AuthAdmin}